...
The mazenginx::resource:: classes and relevant code has been derived from https://github.com/zertico/puppetlabs-nginx.git
...
* Use the mazenginx::vhost define, whose logic and parameters are similar to Example42 apache::vhost
  and where you have to set your docroot and eventually a custom template to use:

        mazenginx::vhost { 'mydomain.com' :
          template => 'myproject/nginx/mydomain/nginx.conf.erb',
          docroot  => '/var/www/mydomain',
        }

* Use the mazenginx::resource::vhost define which has been ported from puppetlabs/nginx module
  and it provides more flexibility in the management of virtual hosts and single location
  statements (with the mazenginx::resource::location define).

* Templates used by mazenginx::resource can be overriden

        mazenginx::resource::vhost { 'mydomain.com' :
...
* You can add additional locations for special handling like redirect:
        ...
        mazenginx::resource::location {"www.example.org-wiki":
          ensure             => present,

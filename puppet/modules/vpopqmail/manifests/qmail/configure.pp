...
#
# This class further configurates the qmail installation
#
# == Author
#   CDS-Internetagentur
#
class vpopqmail::qmail::configure inherits vpopqmail
{
    ...
    file { "set relaying localhost":
        ...
        path        => "/etc/tcp.smtp",
        content     => template('vpopqmail/tcp.smtp.relay.erb')
    }
    ...
    file { "supervise send":
        content     => template("$module_name/send.erb"),
    ...
    file { "supervise send/log":
        content     => template("$module_name/send_log.erb"),
    ...
    file { "supervise smtpd":
        content     => template("$module_name/smtpd_run.erb"),
    ...
    file { "supervise smtpd/log":
        content     => template("$module_name/smtpd_log.erb"),

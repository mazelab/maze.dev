...
#
# This class defines qmail-courier installation
#
# == Author
#   CDS-Internetagentur
#
class vpopqmail::qmail::courier inherits vpopqmail
{
    ...
    file { "apply pop3 configuration":
        content     => template("$module_name/courier.pop3d.erb"),
    ...
    file { "libauthvchkpw to authlib":
        ...
        source      => "puppet:///modules/vpopqmail/authlib/libauthvchkpw.so",
        require     => [Package["imap-ssl", "authlib"]]
    }

    file { "apply authdaemon config":
        content     => template("$module_name/authdaemonrc.erb"),
    ...
    file { "apply ssl certificate parameters":
        content     => template("$module_name/courier.imap.cnf.erb"),
    ...
    file { "apply pop3 ssl configuration":
        content     => template("$module_name/courier.pop3d-ssl.erb"),
    ...
    file { "apply imap ssl configuration":
        content     => template("$module_name/courier.imap-ssl.erb"),
